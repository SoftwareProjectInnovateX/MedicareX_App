  ...
  return (
    <View className="flex-1 bg-primary">
      <View className="px-6 pt-14 pb-4 bg-white border-b border-[#e5e7eb] flex-row justify-between items-center shadow-sm z-10">
        ...
        <Text className="text-xl font-bold text-textPrimary">Help Center</Text>
        <View className="w-10" />
      </View>
      <ScrollView className="flex-1 px-6 pt-6" showsVerticalScrollIndicator={false}>
        <Text className="text-3xl font-black text-textPrimary mb-2">How can we help you?</Text>
        <Text className="text-textSecondary text-sm mb-6">Find answers to frequently asked questions below.</Text>
        
        {faqs.map((faq, index) => (
          <View key={index} className="bg-white p-5 rounded-2xl mb-4 border border-[#e5e7eb]">
            <Text className="font-bold text-lg text-textPrimary mb-2">{faq.q}</Text>
            <Text className="text-textSecondary leading-5">{faq.a}</Text>
          </View>
        ))}
